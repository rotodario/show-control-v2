    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-2xl font-semibold text-slate-900">Herramientas</h2>
            </div>
            <p class="max-w-2xl text-sm text-slate-500">
                Salud del sistema, copias de seguridad de base de datos y utilidades de mantenimiento desde web.
            </p>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap gap-2">
                <div class="flex h-[4.6rem] w-[11.5rem] flex-col justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Usuarios</p>
                    <div class="mt-1 flex items-end justify-between gap-2">
                        <p class="text-xl font-semibold text-slate-900">{{ $metrics['users_total'] }}</p>
                        <p class="text-[10px] text-slate-500">{{ $metrics['users_active'] }} activas</p>
                    </div>
                </div>
                <div class="flex h-[4.6rem] w-[11.5rem] flex-col justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Super Admin</p>
                    <div class="mt-1 flex items-end justify-between gap-2">
                        <p class="text-xl font-semibold text-slate-900">{{ $metrics['super_admins_active'] }}</p>
                        <p class="text-[10px] text-slate-500">activas</p>
                    </div>
                </div>
                <div class="flex h-[4.6rem] w-[11.5rem] flex-col justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Giras</p>
                    <div class="mt-1 flex items-end justify-between gap-2">
                        <p class="text-xl font-semibold text-slate-900">{{ $metrics['tours_total'] }}</p>
                    </div>
                </div>
                <div class="flex h-[4.6rem] w-[11.5rem] flex-col justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Bolos</p>
                    <div class="mt-1 flex items-end justify-between gap-2">
                        <p class="text-xl font-semibold text-slate-900">{{ $metrics['shows_total'] }}</p>
                    </div>
                </div>
                <div class="flex h-[4.6rem] w-[11.5rem] flex-col justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Backups</p>
                    <div class="mt-1 flex items-end justify-between gap-2">
            ...
            <div class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h3 class="text-lg font-semibold text-slate-900">Salud del sistema</h3>
                ...
                <section class="space-y-6">
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-slate-900">Crear backup</h3>
                                <p class="mt-2 text-sm text-slate-500">
                                    Genera una copia JSON de la base de datos de la aplicacion. No incluye archivos subidos de `storage`.
                                </p>
                            </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h3 class="text-lg font-semibold text-slate-900">Backups guardados</h3>
                        <div class="mt-5 space-y-3">
                            ...
                            @empty
                                <div class="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-500">
                                    Todavia no hay backups guardados.
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div class="rounded-3xl border border-rose-200 bg-white p-6 shadow-sm">
                        <h3 class="text-lg font-semibold text-slate-900">Restaurar backup</h3>
                        <p class="mt-2 text-sm text-slate-500">
                            Esta accion reemplaza el contenido de la base de datos de la aplicacion con el backup subido.
                        </p>
                        <p class="mt-2 text-sm font-medium text-rose-700">
                            Es destructiva. Solo para restauraciones conscientes y con copia previa hecha.
                        </p>
                        ...
                        <form method="POST" action="{{ route('platform.tools.restore') }}" enctype="multipart/form-data" class="mt-6 space-y-4">
                            ...
                            <div>
                                <label for="backup_file" class="text-sm font-semibold text-slate-900">Archivo backup JSON</label>
                            ...
                            <div>
                                <label for="confirmation" class="text-sm font-semibold text-slate-900">Escribe `RESTAURAR` para confirmar</label>
