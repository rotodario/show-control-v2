<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Produccion</p>
                <h2 class="text-2xl font-semibold text-slate-900">Calendario de bolos</h2>
                <p class="mt-1 text-sm text-slate-500">Vista mensual clasica con agenda del dia seleccionado.</p>
            </div>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('shows.calendar') }}" class="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm">
                <div class="grid gap-4 lg:grid-cols-[1fr_220px_220px_auto] lg:items-end">
                    <div>
                        <x-input-label for="calendar_tour_filter" value="Filtrar por gira" />
            ...
            <div class="space-y-6">
                <div class="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Mes activo</p>
                                        ...
                                        @foreach ($day['shows']->take(1) as $show)
                                            <div class="rounded-md px-1 py-1 text-[9px] sm:px-1.5 sm:text-[10px] lg:px-2 lg:py-1.5 lg:text-xs {{ count($showAlerts[$show->id] ?? []) > 0 ? 'bg-amber-100 text-amber-900' : 'bg-slate-100 text-slate-800' }}">
                                                <div class="flex items-center">
                                                    ...
                                                    @else
                                                        <span class="truncate font-semibold text-slate-500">Sin gira</span>
                                                    @endif
                                                </div>
                                                <div class="mt-0.5 hidden items-center gap-1 sm:flex lg:gap-1.5">
                                                    <span class="h-1.5 w-1.5 shrink-0 rounded-full lg:h-2 lg:w-2 {{ match ($show->status) { 'closed' => 'bg-emerald-500', 'confirmed' => 'bg-sky-500', 'cancelled' => 'bg-rose-500', default => 'bg-amber-400' } }}"></span>
                ...
                <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between xl:flex-col xl:items-start">
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-500">Agenda</p>
                            <h3 class="mt-2 text-2xl font-semibold text-slate-900">{{ $selectedDate->translatedFormat('l d F Y') }}</h3>
                        </div>
                        <span class="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-600">{{ $agendaShows->count() }} bolos</span>
                    </div>
                    ...
                    <div class="mt-6 grid gap-4">
                                ...
                                <div class="flex flex-col gap-4">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            <span class="rounded-full px-3 py-1 text-xs font-semibold {{ match ($show->status) { 'closed' => 'bg-emerald-100 text-emerald-700', 'confirmed' => 'bg-sky-100 text-sky-700', 'cancelled' => 'bg-rose-100 text-rose-700', default => 'bg-amber-100 text-amber-700' } }}">
                                                {{ $statusOptions[$show->status] ?? $show->status }}
                                            </span>
                                        </div>
                                        <p class="mt-2 text-sm text-slate-500">{{ $show->city }} · {{ $show->venue ?: 'Venue pendiente' }}</p>
                                        @if ($show->tour)
                        ...
                        @empty
                            <div class="rounded-[1.75rem] border border-dashed border-slate-300 p-10 text-center text-sm text-slate-500">
                                No hay bolos en la agenda del dia seleccionado.
                            </div>
                        @endforelse
                    </div>
                </div>
