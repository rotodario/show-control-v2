        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
                @if ($permissions['delete_shows'])
                    <form method="POST" action="{{ route('public-access.shows.destroy', [$grant->token, $show]) }}" onsubmit="return confirm('¿Eliminar este bolo?');">
                        @csrf
                        @method('DELETE')
        ...
        <div class="mt-6 space-y-6">
            @if ($visibility['alerts'] && $alerts !== [])
                <div class="rounded-[2rem] border border-amber-200 bg-amber-50 p-6 shadow-sm">
                    <h2 class="text-lg font-semibold text-amber-950">Alertas</h2>
            ...
            @if ($permissions['update_shows'])
                <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-lg font-semibold text-slate-900">Editar bolo</h2>
            ...
            <div class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                <section class="space-y-6">
                    ...
                    @if ($visibility['lighting'])
                        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                            <h2 class="text-lg font-semibold text-slate-900">Iluminacion</h2>
                            <div class="mt-4 space-y-3 text-sm leading-6 text-slate-600 [&_p]:my-0 [&_strong]:font-semibold [&_strong]:text-slate-900 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5">
                                {!! $show->lighting_notes
                                    ? \Illuminate\Support\Str::markdown($show->lighting_notes, [
                                        'html_input' => 'strip',
                                        'allow_unsafe_links' => false,
                                    ])
                                    : '<p>Sin notas de iluminacion.</p>' !!}
                            </div>
                            @if (in_array('lighting', $chatSections, true))
                                @include('shows.partials.section-chat', [
                                    'section' => 'lighting',
                                    'messages' => $sectionMessages->get('lighting', collect()),
                                    'action' => route('public-access.section-messages.store', [$grant->token, $show]),
                                ])
                            @endif
                        </div>
                    @endif
                    ...
                    @if ($visibility['sound'])
                        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                            <h2 class="text-lg font-semibold text-slate-900">Sonido</h2>
                            <div class="mt-4 space-y-3 text-sm leading-6 text-slate-600 [&_p]:my-0 [&_strong]:font-semibold [&_strong]:text-slate-900 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5">
                                {!! $show->sound_notes
                                    ? \Illuminate\Support\Str::markdown($show->sound_notes, [
                                        'html_input' => 'strip',
                                        'allow_unsafe_links' => false,
                                    ])
                                    : '<p>Sin notas de sonido.</p>' !!}
                            </div>
                            @if (in_array('sound', $chatSections, true))
                                @include('shows.partials.section-chat', [
                                    'section' => 'sound',
                                    'messages' => $sectionMessages->get('sound', collect()),
                                    'action' => route('public-access.section-messages.store', [$grant->token, $show]),
                                ])
                            @endif
                        </div>
                    ...
                    @if ($visibility['space'])
                        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                            <h2 class="text-lg font-semibold text-slate-900">Espacio / venue</h2>
                            <div class="mt-4 space-y-3 text-sm leading-6 text-slate-600 [&_p]:my-0 [&_strong]:font-semibold [&_strong]:text-slate-900 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5">
                                {!! $show->space_notes
                            ...
                            @if (in_array('space', $chatSections, true))
                                @include('shows.partials.section-chat', [
                                    'section' => 'space',
                                    'messages' => $sectionMessages->get('space', collect()),
                                    'action' => route('public-access.section-messages.store', [$grant->token, $show]),
                                ])
                            @endif
                        </div>
                    ...
                    @if ($visibility['general'])
                        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                            <h2 class="text-lg font-semibold text-slate-900">Notas generales</h2>
                            <div class="mt-4 space-y-3 text-sm leading-6 text-slate-600 [&_p]:my-0 [&_strong]:font-semibold [&_strong]:text-slate-900 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5">
                                {!! $show->general_notes
                            ...
                            @if (in_array('general', $chatSections, true))
                                @include('shows.partials.section-chat', [
                                    'section' => 'general',
                                    'messages' => $sectionMessages->get('general', collect()),
                                    'action' => route('public-access.section-messages.store', [$grant->token, $show]),
                                ])
                            @endif
                        </div>
                    ...
                    @if ($visibility['documents'])
                        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                            <div class="flex items-center justify-between gap-3">
                                <h2 class="text-lg font-semibold text-slate-900">Documentos visibles</h2>
                            ...
                            <div class="mt-4 space-y-3">
                                ...
                                @empty
                                    <div class="rounded-2xl border border-dashed border-slate-300 p-6 text-sm text-slate-500">
                                        No hay documentos visibles para este acceso.
                                    </div>
                                @endforelse
                            </div>
                            ...
                            @if ($permissions['upload_documents'])
                                <div class="mt-6 rounded-3xl bg-slate-50 p-5">
                                    <h3 class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Subir documento</h3>
